...
class Tokenizer():
    ...
    def split_on_special_tokens(self,text:str,special_tokens:list[str])->list[str]:
        ...
        # pattern=f"({"|".join([re.escape(st)for st in sorted_tokens])})"
    ...
    def encode_pure(self,tokens:list[str])->list[int]:
        ...
        result=[]
        for token in tokens:
            byte_seq=[bytes([b]) for b in token.encode('utf-8')]
            ...
            while True:
                ...
                # 选择优先级最高的 pair（rank 最小）
                pair = min(pairs, key=lambda item: self.merge_rank[item])
                
                new_byte_seq = []
                i = 0
                while i<len(byte_seq):
                    if i<len(byte_seq)-1 and (byte_seq[i], byte_seq[i + 1]) == pair:
                        new_byte_seq.append(pair[0] + pair[1])
                        i += 2
                    else:
            ...
            result.extend([self.bytes_to_id[tok] for tok in byte_seq])
    ...
    def decode(self,ids:list[int])-> str:
        raw_utf8=b"".join([self.vocab[int(id)] for id in ids])
        return raw_utf8.decode('utf-8',errors='replace')
...
if __name__ == "__main__":
    ...
    tokenizer=Tokenizer.from_files("/workspace/home/luotianwei/cs336/assignment1-basics/training_result/tinystory_vocab.json","/workspace/home/luotianwei/cs336/assignment1-basics/training_result/tinystory_merges.txt",['<|endoftext|>'])
    
    print(tokenizer.encode("I'm luo tianwei ji 31 哈哈 <|endoftext|>"))
    print(tokenizer.decode(tokenizer.encode("I'm luo tianwei ji 31 哈哈 <|endoftext|>")))
    print('aaa')
